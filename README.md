...
├── package.json       # 依赖配置
├── routes/
│   ├── admin.js       # 管理后台 API / 统计图表 / 备份恢复 / 在线更新
│   ├── auth.js        # 前台密码验证
│   ├── frontendAuth.js # 前台访问 Cookie 鉴权
│   ├── notify.js      # 通知触发 API
...
> 后台点击「开始更新」后，更新成功会自动刷新当前后台页面，确保加载最新静态资源。
> 如果本次更新新增或修改了后端接口，页面刷新后仍需要重启 Node/PM2 进程；否则新页面调用新接口时会失败。
